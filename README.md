...
Я для своих целей существенно доработал тестовую программу, подключил LCD, сделал самокалибровку и исправил досадную ошибку калибровки магнитометра.
...
4. Полсле отчета десяти, быстро переворачиваем компас и по истесении 15 секунд происходит запись +Z компоненты и вычисление смещения по формуле offZ = (maxZ + minZ)/2. Если компоненты равны по модулю то оффсет будет нулевым, но это маловероятно.
...
![](Photos/2016-12-26_22-46-22 HMC83L_compens_MPU6050_198_59_-156_Nokia_115_DLPF_v6.0.jpg)
...
## Формат данных на выходе UART компаса.
    ...
    Serial.print(",");  //разделитель для парсера
    Serial.println(floor(Xh)+floor(Yh)); //контрольная сумма
